...
def create_entity(db: Session, user_id: str, entity: str, value: str):
    db_entity = models.Entity(owner_id=user_id, entity=entity, value=value)
    db.add(db_entity)
    db.commit()
...
def entity_exist(db: Session, user_id: str, entity: str):
    db_entity = (db.query(models.Entity)
                 .filter(models.Entity.owner_id == user_id)
                 .filter(models.Entity.entity == entity)
                 .first())
    if db_entity:
        return db_entity
    else:
...
def update_or_create_entity(db: Session, user_id: str, entity: str, value: str):
    db_entity = entity_exist(db, user_id, entity)
    if db_entity:
        return db_entity
    else:
        return create_entity(db, user_id, entity, value)


def delete_entity(db: Session, user_id: str, entity: str):
    db_entity = entity_exist(db, user_id, entity)
    if db_entity:
        db.delete(db_entity)
        db.commit()
